	  ...
	  struct task_struct *tsk)
{
	...
	if (!cpumask_test_and_set_cpu(cpu, mm_cpumask(next)) || prev != next) {
		...
		unsigned int stack_pgd_index[] = {
			pgd_index((unsigned long)task_stack_page(tsk)),
			pgd_index(current_stack_pointer),
		};

		for (i = 0; i < ARRAY_SIZE(stack_pgd_index); i++) {

			pgd_t *pgd = next->pgd + stack_pgd_index[i];

			if (unlikely(pgd_none(*pgd)))
				set_pgd(pgd, init_mm.pgd[stack_pgd_index[i]]);
		}
#endif
