...
static bool disable_power_save_mode = false;
				     ...
				     u32 pwr_save, u32 ignore_dat0)
{
	...
	dev_info(mmc_dev(host->mmc), "=============== ignore_dat0 = %d oclk_en = %d pwr_save = %d\n",ignore_dat0, oclk_en,pwr_save);
...
void sunxi_mmc_disable_power_save_mode_v4p1x(void){
	disable_power_save_mode = true;
}
EXPORT_SYMBOL_GPL(sunxi_mmc_disable_power_save_mode_v4p1x);
