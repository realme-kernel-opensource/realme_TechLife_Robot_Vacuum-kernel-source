...
static int aw2016_led_parse_child_node(struct aw2016_led *led_array,int led_count)
{
    struct aw2016_led *led;
	...
	for(parsed_leds=0;parsed_leds<led_count;parsed_leds++){
        ...
        rc = of_property_read_string(temp, "led_name",
            &led->cdev.name);
        if (rc < 0) {
            dev_err(&led->client->dev,
        ...
        rc = of_property_read_u32(temp, "id",
            &led->id);
        if (rc < 0) {
            dev_err(&led->client->dev,
        ...
        rc = of_property_read_u32(temp, "imax",
            &led->pdata->imax);
        if (rc < 0) {
            dev_err(&led->client->dev,
...
static int aw2016_led_detect(struct i2c_client *client, struct i2c_board_info *info)
{
