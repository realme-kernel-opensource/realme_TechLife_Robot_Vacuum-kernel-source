...
static int ssv6xxx_hci_set_trigger_qlen(struct ssv6xxx_hci_ctrl *hci_ctrl, u32 qlen)
{
...
static int ssv6xxx_hci_enqueue(struct ssv6xxx_hci_ctrl *hci_ctrl, struct sk_buff *skb, int txqid, u32 tx_flags)
{
    ...
    if (0 != hci_ctrl->tx_trigger_qlen) {
        // check data txq len to trigger task
        if ((txqid < (SSV_SW_TXQ_NUM-1)) && (qlen < hci_ctrl->tx_trigger_qlen))
            return qlen;
    }
...
static struct ssv6xxx_hci_ops hci_ops = 
{
    ...
    .hci_set_trigger_qlen = ssv6xxx_hci_set_trigger_qlen,
};
