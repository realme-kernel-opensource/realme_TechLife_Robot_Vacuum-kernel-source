...
static struct device *wdev = NULL;
extern struct device *sunxi_wlan_get_dev(void);
extern int sunxi_wlan_set_suspend_flag(int val);
#endif
/* static int irq_thread_wifi_suspend_flag; */
static irqreturn_t gpio_hostwakeup_irq_thread(int irq, void *data)
{
	static bool irq_thread_wlan_first_wakeup = true;
	PADAPTER padapter = (PADAPTER)data;

	if( irq_thread_wlan_first_wakeup ){
		irq_thread_wlan_first_wakeup = false;
	}else{
#ifdef CONFIG_PLATFORM_ARM_RK3188
		rockchip_rtlwifi_set_wakeup_status(1);
#endif

#ifdef CONFIG_PLATFORM_ARM_SUNxI
		sunxi_wlan_set_suspend_flag(1);
#endif
		RTW_PRINT("rtlwifi_set_wakeup_status === 2020.08.18 by hfh@ldrobot.com\n");
	}

#ifdef CONFIG_PLATFORM_ARM_SUNxI
    if (wdev && device_may_wakeup(wdev)) {
            pm_wakeup_event(wdev, 0);
            RTW_PRINT("pm_wakeup_event...hfh\n");
    }else{
            RTW_PRINT("not pm_wakeup_event...hfh[wdev%p]\n",wdev);
    }
#endif	
...
static int __init rtw_drv_entry(void)
{
    ...
    RTW_INFO("%s: add by hfh 2020-10-27\n", __FUNCTION__);
#endif
