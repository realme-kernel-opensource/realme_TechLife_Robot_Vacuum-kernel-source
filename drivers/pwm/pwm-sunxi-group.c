		...
		int duty_ns, int period_ns)
{
	...
	pc->start_count = entire_cycles / 4;
	...
	if (pc->g_channel) {
		...
		if (pc->g_channel == 1) {
			...
			sunxi_pwm_writel(chip, reg_offset, temp);
		} else if (pc->g_channel == 2) {
			reg_offset = PGR1;
			reg_shift = sel;
			reg_width = 1;
			temp = sunxi_pwm_readl(chip, reg_offset);
			temp = SET_BITS(reg_shift, reg_width, temp, 1); /* set group1_cs */
			sunxi_pwm_writel(chip, reg_offset, temp);
		}

		/* pwm pulse mode */
		reg_offset = PWM_PCR_BASE + sel * 0x20;
		reg_shift = PWM_MODE_SHIFT;
		reg_width = PWM_MODE_WIDTH;
		temp = sunxi_pwm_readl(chip, reg_offset);
		temp = SET_BITS(reg_shift, 1, temp, 1); /* pwm pulse mode */
		/* pwm output pulse num */
		reg_shift = PWM_PUL_NUM_SHIFT;
		reg_width = PWM_PUL_NUM_WIDTH;
		temp = SET_BITS(reg_shift, reg_width, temp, (pwm_run_count - 1));   /* pwm output pulse num */
		sunxi_pwm_writel(chip, reg_offset, temp);
...
static int sunxi_pwm_enable_single(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	static unsigned int enable_num, pwm_start_count;
	...
	if (pc->g_channel) {
		enable_num++;
		/* start count set */
		reg_offset = PWM_PCNTR_BASE + 0x20 * index;
		...
		pwm_start_count += pc->start_count;
	}
	...
	if (pc->g_channel && enable_num == 4) {
		if (pc->g_channel == 1)
			group_reg_offset = PGR0;
		else if (pc->g_channel == 2)
			group_reg_offset = PGR1;
