...
struct sensor_indetect {
	...
	struct work_struct tp9950_work;
	dev_t devno;
} sensor_indetect;
...
{
	schedule_work(&sensor_indetect.tp9950_work);
	return IRQ_HANDLED;
}
#endif
...
static int tp9950_sensor_det_init(struct i2c_client *client)
{
	...
	INIT_WORK(&sensor_indetect.tp9950_work, sensor_det_work);
...
{
	cancel_work_sync(&sensor_indetect.tp9950_work);
