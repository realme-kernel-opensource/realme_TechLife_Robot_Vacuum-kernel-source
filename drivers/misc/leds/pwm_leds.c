...
static unsigned int led_brightness;
#define red_shift 16
...
static void set_pwm(int pwm_id, int level)
{
	...
	period_ns = 20000; /*1000000000/50000 freq=50KHz, 1s=1000000000ns*/
	duty_ns = (period_ns * level)/MAX ;
	if (pwm_id == 0) {
		ret = pwm_config(t_pwm_red_led->pwm0_dev, duty_ns, period_ns);
		ret = pwm_enable(t_pwm_red_led->pwm0_dev);
	} else if (pwm_id == 1) {
		ret = pwm_config(t_pwm_green_led->pwm1_dev, duty_ns, period_ns);
		ret = pwm_enable(t_pwm_green_led->pwm1_dev);
	} else if (pwm_id == 2) {
		ret = pwm_config(t_pwm_blue_led->pwm2_dev, duty_ns, period_ns);
		ret = pwm_enable(t_pwm_blue_led->pwm2_dev);
	}
		...
		const char *buf, size_t size)
{
	...
	if (t_pwm_red_led->time_delay != 0)
		cancel_delayed_work_sync(&red_led_delay_work);

	pwm_printf("id = %d, status= %d\n",
			t_pwm_red_led->pwm_id, t_pwm_red_led->red_status);
	switch (t_pwm_red_led->red_status) {
	case 0: /*LOW_BRIGHT*/
		t_pwm_red_led->time_delay = 0;
		set_pwm(t_pwm_red_led->pwm_id, 90);
		break;
	case 1: /*MIDIUM_BRIGHT*/
		t_pwm_red_led->time_delay = 0;
		set_pwm(t_pwm_red_led->pwm_id, 50);
		break;
	case 2: /*HIGHT_BRIGHT*/
		t_pwm_red_led->time_delay = 0;
		set_pwm(t_pwm_red_led->pwm_id, 10);
		break;
	case 4: /*DISABLE*/
		t_pwm_red_led->time_delay = 0;
		pwm_disable(t_pwm_red_led->pwm0_dev);
		break;
	case 6: /*ADJEST_BRGHT*/
		t_pwm_red_led->time_delay = HZ / 2;
		break;
	default:
		t_pwm_red_led->time_delay = 0;
		pr_err("inval cmd\n");
	...
	if (t_pwm_red_led->time_delay)
		schedule_delayed_work(&red_led_delay_work,
		t_pwm_red_led->time_delay);
	return size;
}
		...
		const char *buf, size_t size)
{
	...
	case 4: /*DISABLE*/
		t_pwm_green_led->time_delay = 0;
		pwm_disable(t_pwm_green_led->pwm1_dev);
		break;
	case 6: /*ADJEST_BRGHT*/
		...
		const char *buf, size_t size)
{
	...
	case 4: /*DISABLE*/
		t_pwm_blue_led->time_delay = 0;
		pwm_disable(t_pwm_blue_led->pwm2_dev);
		break;
	case 6: /*ADJEST_BRGHT*/
...
static ssize_t get_led_func(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%x\n", led_brightness);
}

static ssize_t set_led_func(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t size)
{
	int ret;
	int rstat, gstat, bstat;
	unsigned long tmp;

	ret = kstrtoul(buf, 0, &tmp);
	if (ret < 0) {
		pwm_printf("input err!,buf = %s\n", buf);
		return -1;
	}
	led_brightness = tmp;

	rstat = MAX - ((led_brightness>>red_shift)&0xff);
	gstat = MAX - ((led_brightness>>green_shift)&0xff);
	bstat = MAX - ((led_brightness>>blue_shift)&0xff);
	pwm_printf("led_brightness = 0x%x, rstat = %d, gstat = %d, "
			"bstat = %d \n", led_brightness, rstat, gstat, bstat);

	set_pwm(t_pwm_red_led->pwm_id, rstat);
	set_pwm(t_pwm_green_led->pwm_id, gstat);
	set_pwm(t_pwm_blue_led->pwm_id, bstat);
	return size;
}
...
static DEVICE_ATTR(led_ctrl, 0664,
		get_led_func, set_led_func);
...
static void pwm_red_led_delay_work(struct work_struct *work)
{
	static int level;
	static int pol;
	int pwm_id = t_pwm_red_led->pwm_id;
...
/*	case 23: set_pwm(pwm_id, 8); break;*/
	...
	if (level == 24)
		schedule_delayed_work(&red_led_delay_work, HZ/1);
	else if (level == 0)
		schedule_delayed_work(&red_led_delay_work, HZ/2);
	else
		schedule_delayed_work(&red_led_delay_work, HZ/(level+4));
}
...
static int pwm_leds_ctrl_init(void)
{
	...
	t_pwm_red_led->pwm0_dev = pwm_request(PWM_NUM_RED, "led_red");
	if (t_pwm_red_led->pwm0_dev ==  NULL ||
			IS_ERR(t_pwm_red_led->pwm0_dev)) {
		pr_err("[pwm_leds] pwm_request pwm0 fail!\n");
		return -1;
	}
	t_pwm_red_led->red_status = LED_DEFAULT_STATE;
	t_pwm_red_led->pwm_id = 0;
	t_pwm_red_led->time_delay = LED_DEFAULT_DELAY;

	pwm_printf("[pwm_leds] pwm_request pwm0 success!\n");

	t_pwm_green_led->pwm1_dev = pwm_request(PWM_NUM_GREEN, "led_green");
	if (t_pwm_green_led->pwm1_dev == NULL ||
			IS_ERR(t_pwm_green_led->pwm1_dev)) {
		pr_err("[pwm_leds] pwm_request pwm1 fail!\n");
		return -1;
	}
	t_pwm_green_led->green_status = LED_DEFAULT_STATE;
	t_pwm_green_led->pwm_id = 1;
	t_pwm_green_led->time_delay = LED_DEFAULT_DELAY;

	pwm_printf("[pwm_leds] pwm_request pwm1 success!\n");

	t_pwm_blue_led->pwm2_dev = pwm_request(PWM_NUM_BLUE, "led_blue");
	if (t_pwm_blue_led->pwm2_dev ==  NULL ||
			IS_ERR(t_pwm_blue_led->pwm2_dev)) {
		pr_err("[pwm_leds] pwm_request pwm2 fail!\n");
		return -1;
	}
	t_pwm_blue_led->blue_status = LED_DEFAULT_STATE;
	t_pwm_blue_led->pwm_id = 2;
	t_pwm_blue_led->time_delay = LED_DEFAULT_DELAY;

	pwm_printf("[pwm_leds] pwm_request pwm2 success!\n");
/*
...
*/
	...
	INIT_DELAYED_WORK(&red_led_delay_work, pwm_red_led_delay_work);
...
static void __exit pwm_leds_exit(void)
{
	...
	cancel_delayed_work_sync(&green_led_delay_work);
	cancel_delayed_work_sync(&blue_led_delay_work);
	pwm_free(t_pwm_red_led->pwm0_dev);
	pwm_free(t_pwm_green_led->pwm1_dev);
	pwm_free(t_pwm_blue_led->pwm2_dev);
	kfree(t_pwm_red_led);
...
MODULE_DESCRIPTION("a simple pwmleds driver, 3 pwms control 3 color light");
MODULE_AUTHOR("kirin");
MODULE_LICENSE("GPL");
