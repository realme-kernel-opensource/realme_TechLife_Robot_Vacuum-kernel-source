...
int wifi_suspend_flag = 0;
int sunxi_wlan_get_suspend_flag(void)
{
	printk("%s->%d get wifi_suspend_flag=%d\n", __func__, __LINE__, wifi_suspend_flag);

	return wifi_suspend_flag;
}
EXPORT_SYMBOL_GPL(sunxi_wlan_get_suspend_flag);

int sunxi_wlan_set_suspend_flag(int val)
{
	wifi_suspend_flag = val;
	printk("%s->%d set ifi_suspend_flag=%d\n", __func__, __LINE__, wifi_suspend_flag);

	return wifi_suspend_flag;
}
EXPORT_SYMBOL_GPL(sunxi_wlan_set_suspend_flag);
	...
	for (i = 0; i < (data->power_num); i++) {
		if (data->wlan_power_name[i]) {
			...
			if (!IS_ERR(data->wlan_power[i])) {
				if (on_off) {
					ret =
					    regulator_enable(data->wlan_power
							     [i]);
					if (ret < 0) {
						dev_err(dev,
					...
					dev_info(dev,
						 "current check wlan wlan_power voltage: %d\n",
						 ret);
					if (ret < data->wlan_power_voltage) {
						ret =
						    regulator_set_voltage(data->
									  ...
									  wlan_power_voltage);
						dev_info(dev,
							 "set wlan wlan_power voltage: %d\n",
							 data->
							 wlan_power_voltage);
					}
				} else {
					ret =
					    regulator_disable(data->wlan_power
							      [i]);
					if (ret < 0) {
						dev_err(dev,
	...
	if (data->io_regulator_name) {
		...
		if (!IS_ERR(data->io_regulator)) {
			if (on_off) {
				...
				dev_info(dev,
					 "current check wlan io regulator voltage: %d\n",
					 ret);
				if (ret < data->wlan_io_voltage) {
					ret =
					    regulator_set_voltage(data->
								  io_regulator,
								  data->
								  ...
								  wlan_io_voltage);
					dev_info(dev,
						 "set wlan io regulator voltage: %d\n",
						 data->wlan_io_voltage);
				}
			} else {
...
static DEVICE_ATTR(scan_device, S_IRUGO | S_IWUSR, NULL, scan_device_store);
...
static int sunxi_wlan_probe(struct platform_device *pdev)
{
	...
	if (!gpio_is_valid(data->gpio_wlan_regon)) {
		...
		ret = gpio_direction_output(data->gpio_wlan_regon, 1);
		if (ret < 0) {
			dev_err(dev,
	...
	data->gpio_chip_en = of_get_named_gpio_flags(np,
						     "chip_en", 0,
						     (enum of_gpio_flags *)
						     &config);
	if (!gpio_is_valid(data->gpio_chip_en)) {
		...
		dev_info(dev,
			 "chip_en gpio=%d  mul-sel=%d pull=%d  drv_level=%d  data=%d\n",
		...
		if (ret < 0) {
			dev_err(dev,
				"can't request output direction chip_en gpio %d\n",
				data->gpio_chip_en);
			goto end;
		}
	}
	...
	if (ret) {
		dev_err(dev,
			"sunxi-wlan register driver as misc device error!\n");
		goto end;
	}
	...
	if (ret) {
		dev_err(dev,
			"sunxi-wlan register sysfs create group failed!\n");
		goto end;
	}
