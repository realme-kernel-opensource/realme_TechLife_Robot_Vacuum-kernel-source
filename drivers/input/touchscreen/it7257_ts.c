				...
				bool forever, bool slowly)
{
	...
	do {
		ret = it7259_i2c_read_no_ready_check(ts_data, BUF_QUERY, &query, sizeof(query));
		if (ret < 0 && ((query & CMD_STATUS_BITS) == CMD_STATUS_NO_CONN))
			continue;
		...
		unsigned char readDataBuffer[], unsigned short readDataLength)
{
	...
	unsigned char buffer4Write[1024];
	struct i2c_msg msgs[2] = {
		{.addr = ts_data->client->addr, .flags = 0, .len = 4, .buf = buffer4Write},
		{.addr = ts_data->client->addr, .flags = I2C_M_RD, .len = readDataLength, .buf = readDataBuffer}
	};
			...
			unsigned short readDataLength)
{
	...
	unsigned char buffer4Write[1024];
	struct i2c_msg msgs[2] = {
					{ .addr = ts_data->client->addr, .flags = 0, .len = 2, .buf = buffer4Write},
					{ .addr = ts_data->client->addr, .flags = I2C_M_RD, .len = readDataLength, .buf = readDataBuffer}
				};
				...
				struct it7259_ts_platform_data *pdata)
{
	...
	pdata->reset_gpio = of_get_named_gpio_flags(np,
			"reset-gpios", 0, &pdata->reset_gpio_flags);
	pdata->irq_gpio = of_get_named_gpio_flags(np,
			"int-gpios", 0, &pdata->irq_gpio_flags);
