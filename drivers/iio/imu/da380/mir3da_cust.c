...
struct gpio_config gsensor_int;
int gsensor_irq;
...
static void initDA380Reg(void)
{
	...
	res |=
	    mir3da_register_mask_write(handle, NSA_REG_INT_LATCH, 0x06, 0x06);
	/* 83 1s 84 2s 85 4s  86 8s  8f yiz */
	res |= mir3da_register_mask_write(handle, NSA_REG_ACTIVE_DURATION, 0xff,
					  0x03);
	res |= mir3da_register_mask_write(handle, NSA_REG_ACTIVE_THRESHOLD,
			...
			const struct i2c_device_id *id)
{
	...
	printk("ParkMonitor hwmon_device_register  \n");
	...
	gsensor_int.gpio = of_get_named_gpio_flags(
	    np, "gsensor_int1", 0, (enum of_gpio_flags *)(&(gsensor_int)));

	printk("ParkMonitor gsensor_int.gpio=%d  \n", gsensor_int.gpio);

	if (!gpio_is_valid(gsensor_int.gpio) || gsensor_int.gpio < 0) {
		MI_ERR("%s: gsensor_int is invalid.\n", __func__);
	} else {

		result = gpio_request(gsensor_int.gpio, NULL);
		if (result < 0) {
			MI_ERR("can't request m_wakeup_ap gpio %d\n", gsensor_int.gpio);
			return result;
		}

		gpio_direction_input(gsensor_int.gpio);
		gsensor_irq = gpio_to_irq(gsensor_int.gpio);
		...
		if (result) {
			...
			enable_wakeup_src(CPUS_GPIO_SRC, gsensor_int.gpio);
#else
			...
			printk("ParkMonitor gsensor_int.gpio=%d  gsensor_irq=%d  \n",
					 gsensor_int.gpio, gsensor_irq);
#endif
		}
...
static int gsensor_fetch_sysconfig_para(void)
{
	...
	struct device_node *np = NULL;
	np = of_find_node_by_name(NULL, "gsensor_para");
	ret = of_property_read_u32(np, "gsensor_twi_id", &twi_id);
	if (ret) {
		pr_err("gsensor get twi_id is fail, %d\n", ret);
		goto script_get_err;
	}
	pr_err("gsensor gsensor_twi_id = %d \n", twi_id);
