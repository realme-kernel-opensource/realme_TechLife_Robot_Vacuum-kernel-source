...
static int sunxi_spdif_resume(struct snd_soc_dai *dai)
{
	...
	if (sunxi_spdif->pllclk != NULL) {
		...
		pr_alert("[sunxi-spdif] pllclk prepare enable error:%d\n", ret);
	}
#ifdef SPDIF_PLL_AUDIO_X4
	if (sunxi_spdif->pllclkx4 != NULL) {
		ret = clk_prepare_enable(sunxi_spdif->pllclkx4);
		pr_alert("[sunxi-spdif] pllclkx4 prepare enable error:%d\n", ret);
	}
#endif
	if (sunxi_spdif->moduleclk != NULL) {
		clk_prepare_enable(sunxi_spdif->moduleclk);
		pr_alert("[sunxi-spdif] modlule clk prepare enable error:%d\n", ret);
	}
